"""
Activation patching on economic uncertainty classification (HIGH/LOW labels).
...
Patching logic:
  ...
  - For each layer, patch the residual stream from 20 tokens before
    the test-statement's final period token through the last token.
...
def build_messages(
    ...
    Always includes the uncertainty definition (same as synthetic).
    """
    demos_block = (
        f"Statement: {certain_demo}\nLabel: LOW\n\n"
        f"Statement: {uncertain_demo}\nLabel: HIGH\n\n"
    )
...
def get_label_token_ids(tokenizer):
    ...
    variants, not ' HIGH'/' LOW' which are different BPE tokens.
    """
...
PRE_PERIOD_TOKENS = 20  # number of tokens before the period to include
...
def run_patching_for_pair(
    ...
    """
    Patch at every token position from PRE_PERIOD_TOKENS before the period
    to the last token.
    ...
    # Start position: up to PRE_PERIOD_TOKENS before the period, clamped to 0
    clean_start_pos   = max(0, clean_period_pos   - PRE_PERIOD_TOKENS)
    corrupt_start_pos = max(0, corrupt_period_pos - PRE_PERIOD_TOKENS)

    # Number of positions from start to last token (inclusive on both ends)
    ...
    # How many tokens are before the period in the patching window
    clean_pre_period   = clean_period_pos   - clean_start_pos
    corrupt_pre_period = corrupt_period_pos - corrupt_start_pos

    # Token labels for the positions (from the corrupt prompt for display)
    token_labels = [
        repr(tokenizer.decode([corrupt_ids[corrupt_start_pos + offset]]))
        for offset in range(n_positions)
    ]
    print(f"  Patching positions: {n_positions} tokens "
          f"(from {min(clean_pre_period, corrupt_pre_period)} before period to end)")
...
def main():
    ...
    parser.add_argument(
        ...
        help="logit_diff: logit(HIGH)-logit(LOW). "
             "prob_diff: P(HIGH)-P(LOW) after softmax over full vocabulary.",
    )
    ...
    # Period-token column: PRE_PERIOD_TOKENS offset into the window
    # (may be less if some pairs had fewer than PRE_PERIOD_TOKENS before the period)
    period_col = min(PRE_PERIOD_TOKENS, min_positions - 1)
    mean_patched_period = mean_patched[:, period_col]  # (n_layers,)

    # ── Plot 1: Period-token line plot (original) ─────────────────────────────
    metric_label = "logit(HIGH) − logit(LOW)" if args.metric == "logit_diff" else "P(HIGH) − P(LOW)"
    ...
    ax.set_xlabel(f"Token position ({PRE_PERIOD_TOKENS} before period → end)")
    ax.set_ylabel("Layer")
