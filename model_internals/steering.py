"""
RepE Steering Pipeline for Economic Uncertainty (v2: HIGH/LOW labels).
...
Reuses prompt utilities from activation_patching_v2.py.
"""
...
from activation_patching_v2 import (
    build_messages,
    ...
    for alpha in alphas:
        ...
        for i, pair in enumerate(test_pairs):
            for label_key, true_label in [("certain", "LOW"), ("uncertain", "HIGH")]:
                ...
                pred_label = "HIGH" if logit_high > logit_low else "LOW"
        ...
        print(f"    Accuracy: {accuracy:.4f}  |  Uncertain: {high_acc:.4f}  |  Certain: {low_acc:.4f}  |  Mean logit diff: {mean_logit_diff:.3f}")
...
def main():
    parser = argparse.ArgumentParser(description="RepE Steering for Economic Uncertainty (v2: HIGH/LOW)")
    ...
    print("=" * 60)
    print("RepE Steering Pipeline -- Economic Uncertainty (v2: HIGH/LOW)")
    print("=" * 60)
    ...
    if args.dataset == "templated":
        # Pair 1 = demo (same as activation_patching_v2.py)
        demo_pair = pairs[0]
